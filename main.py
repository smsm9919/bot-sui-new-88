...
def manage_after_entry_enhanced(df, ind, info):
    ...
    if pnl_pct <= HARD_STOP_LOSS_PCT:
        log_w(f"🛑 HARD SL HIT: pnl={pnl_pct:.2f}% <= {HARD_STOP_LOSS_PCT}% → STRICT CLOSE")
...
def trade_loop_professional_with_smc():
    ...
    while True:
        try:
            ...
            # قرار الدخول باستخدام المجلس المحترف مع SMC + Zone/ADX/Liquidity/Cross Gates
            reason = None
            if spread_bps is not None and spread_bps > MAX_SPREAD_BPS:
                reason = f"انتشار مرتفع ({fmt(spread_bps,2)}bps > {MAX_SPREAD_BPS})"

            # نستخدم المجلس اللي في الـ SNAP مباشرة
            council_data = snap.get("cv", {}) or ultimate_council_professional(df)
            smc_data = council_data.get("advanced_indicators", {}).get("smc_analysis", {}) or {}

            # قرار التداول الخام من المجلس (قبل الـ Gates)
            decision = None
            ...
            score_b = float(council_data.get("score_b", 0.0))
            score_s = float(council_data.get("score_s", 0.0))
            votes_b = int(council_data.get("b", 0))
            votes_s = int(council_data.get("s", 0))

            if score_b >= min_confidence and (score_b > score_s + 2.0):
                decision = "BUY"
            elif score_s >= min_confidence and (score_s > score_b + 2.0):
                decision = "SELL"
            ...
            if decision:
                ...
                if market_structure.get("bos_bullish") and decision == "BUY":
                    print("🎯 SMC CONFIRMATION: BOS صاعد يدعم الشراء", flush=True)
                elif market_structure.get("bos_bearish") and decision == "SELL":
                    print("🎯 SMC CONFIRMATION: BOS هابط يدعم البيع", flush=True)

            # تطبيق THE GATE 🔒 (Zone + Liquidity + ADX/ATR Cycle + Cross Strength)
            gate_info = None
            if decision and reason is None:
                # side string للـ Gate
                side_for_gate = "BUY" if decision == "BUY" else "SELL"
                allowed, gate_info = evaluate_entry_gates(df, side_for_gate, council_data, smc_data, snap)

                if not allowed:
                    reason = f"GATE_BLOCKED: {gate_info.get('reason')}"
                    z = gate_info.get("zone") or {}
                    adx = gate_info.get("adx") or {}
                    liq = gate_info.get("liquidity")
                    cs = gate_info.get("cross_strength")

                    print(f"🔒 ENTRY BLOCKED [{decision}] → {reason}", flush=True)
                    if z:
                        print(f"   • Zone: grade={z.get('grade')} strength={z.get('zone', {}).get('strength')}", flush=True)
                    if adx:
                        print(f"   • ADX: {adx.get('phase')} value={adx.get('adx'):.1f}", flush=True)
                    if liq:
                        print(f"   • Liquidity: {liq}", flush=True)
                    if cs is not None:
                        print(f"   • CrossStrength: {cs:.1f}", flush=True)

            # فتح الصفقة فقط لو:
            #   • مافيش صفقة مفتوحة
            #   • decision موجود
            #   • مافيش سبب رفض (reason is None)
            #   • بوابة الانتظار بعد الإغلاق (wait_gate_allow) تسمح
            if not STATE["open"] and decision and reason is None:
                allow_wait, wait_reason = wait_gate_allow(df, info)
                if not allow_wait:
                    reason = wait_reason
                    print(f"⏳ WAIT-GATE BLOCKED: {reason}", flush=True)
                else:
