...
def select_profit_profile(trade_mode, analysis):
    """اختيار الـ Profile المناسب بناءً على تحليل السوق (مع ADX 24–29)"""
    rr = float(analysis.get("edge_rr", 1.0))
    if analysis.get("edge_setup") and analysis["edge_setup"].get("valid"):
        rr = float(analysis["edge_setup"].get("rr1", rr))
    ...
    golden_type = (analysis.get("golden_zone") or {}).get("type")

    # 1) TRAP قوي
    if trade_mode == "TRAP" and stop_q >= 3.0:
        return "TRAP_TREND"

    # 2) Golden أو RR عالي أو ثقة عالية أو ADX قوي ⇒ ترند كامل
    if golden_type in ("golden_bottom", "golden_top") or adx >= 29 or rr >= 2.0 or conf >= 7.0:
        return "FULL_TREND"
...
class ProfitEngine:
    """محرك جني الأرباح والوقف المتحرك الذكي (ديناميكي بالكامل)"""

    def __init__(self, exchange, state):
        # exchange: كائن التنفيذ (BingX / Bybit wrapper)
        # state:   StateManager (يُعامل كـ dict)
        ...
        self.sl_price = None          # وقف الخسارة الحالي (يتحرك مع الترند)
        self.tp_levels = []           # [{"price":..,"frac":..,"label":..,"hit":False}, ...]
    ...
    def _log_plan(self):
        tps = [(round(tp["price"], 6), f"{tp['frac']*100:.0f}%", tp["label"]) for tp in self.tp_levels]
        log_i(
            f"🎯 PROFIT PLAN [{self.profile_name}] | "
            f"side={self.side} | entry={self.entry_price:.6f} | ATR={self.atr_entry:.6f} | "
            f"SL={self.sl_price:.6f} | TPs={tps}"
        )

    def calculate_atr(self, df, period: int = 14):
        if df is None or len(df) < period + 1:
            return self.atr_entry or (self.entry_price * 0.01)
        high = df["high"].astype(float)
        low = df["low"].astype(float)
        close = df["close"].astype(float)
        trs = []
    ...
    def init_trade(self, side: str, entry_price: float, atr_value: float, trade_mode: str, analysis: dict):
        """يتم استدعاؤها مرة واحدة بعد فتح الصفقة"""
        self.side = side.lower()
        self.entry_price = float(entry_price)
        self.atr_entry = float(atr_value or (entry_price * 0.01))

        # اختيار البروفايل (SCALP / MID / FULL / TRAP)
        self.profile_name = select_profit_profile(trade_mode, analysis)
        self.profile_cfg = PROFIT_PROFILES[self.profile_name]

        direction = self._direction()

        # ستوب ابتدائي based on R * ATR
        hard_sl_rr = float(self.profile_cfg["hard_sl_rr"])
        ...
        # بناء مستويات TP
        self.tp_levels = []
        for idx, (rr, frac) in enumerate(zip(self.profile_cfg["tp_levels_rr"], self.profile_cfg["tp_fracs"]), start=1):
            tp_price = self.entry_price + direction * rr * self.atr_entry
            self.tp_levels.append(
                {"idx": idx, "price": tp_price, "frac": float(frac), "label": f"TP{idx}", "hit": False}
            )

        self.trail_active = False
        self.trail_price = None

        # تحديث حالة البوت العامة
        self.state.update(
            profit_profile=self.profile_name,
            profit_engine_active=True,
            sl=self.sl_price,
            tp_levels=[(tp["price"], tp["frac"], tp["label"]) for tp in self.tp_levels],
            trade_profile=self.profile_name,
        )
    ...
    def _update_trailing(self, df, current_price: float, R_now: float):
        trail_start = self.profile_cfg.get("trail_start_rr")
        trail_mult = self.profile_cfg.get("trail_atr_mult")
        if not trail_start or not trail_mult:
            return

        if R_now < trail_start:
            return

        atr_now = self.calculate_atr(df)
        if self.side == "long":
            new_sl = current_price - trail_mult * atr_now
            if new_sl > (self.sl_price or -1e9):
                self.sl_price = new_sl
                self.trail_active = True
                self.trail_price = new_sl
                log_i(f"🔒 TRAIL UPDATE (LONG) | SL→{self.sl_price:.6f}")
        else:
            new_sl = current_price + trail_mult * atr_now
            if new_sl < (self.sl_price or 1e9):
                self.sl_price = new_sl
                self.trail_active = True
                self.trail_price = new_sl
                log_i(f"🔒 TRAIL UPDATE (SHORT) | SL→{self.sl_price:.6f}")

        self.state.update(sl=self.sl_price, dynamic_sl=self.sl_price)

    # ============ تنفيذ TPs + SL ============

    def _process_tps(self, current_price: float):
        ...
        for tp in self.tp_levels:
            ...
            hit = (side == "long" and current_price >= tp_price) or (side == "short" and current_price <= tp_price)
            if not hit:
                new_tp_levels.append(tp)
                continue
            ...
            if self.exchange.execute_order(order_side, close_qty, current_price):
                qty_after = qty - close_qty
                self.state["qty"] = qty_after
                tp["hit"] = True
                closed_any = True

                log_g(
                    f"🎯 {tp['label']} HIT | price={current_price:.6f} | "
                    f"closed={close_qty:.4f} | remain={qty_after:.4f}"
                )

                # بعد أول TP → Breakeven
                if self.profile_cfg.get("be_after_tp") and sum(1 for t in self.tp_levels if t["hit"]) == 1:
                    if side == "long":
                        self.sl_price = self.entry_price * 1.0001
                    else:
                        self.sl_price = self.entry_price * 0.9999
                    log_i(f"⚖️ MOVE SL → BE | SL={self.sl_price:.6f}")
                    self.state.update(sl=self.sl_price)

                qty = qty_after

            new_tp_levels.append(tp)

        self.tp_levels = new_tp_levels
        self.state.update(
            tp_levels=[(tp["price"], tp["frac"], tp["label"]) for tp in self.tp_levels],
        )
        return closed_any

    def _check_sl(self, current_price: float):
        if self.sl_price is None:
            return False

        side = self.side
        hit = (side == "long" and current_price <= self.sl_price) or (
            side == "short" and current_price >= self.sl_price
        )
        if not hit:
            return False
        ...
        order_side = "sell" if side == "long" else "buy"
        if self.exchange.execute_order(order_side, qty, current_price):
            log_w(
                f"⛔ SL HIT | side={side} | price={current_price:.6f} | "
                f"entry={self.entry_price:.6f} | qty={qty:.4f}"
            )
            self.state.update(open=False, qty=0.0, profit_engine_active=False)
            return True

        return False

    # ============ الحلقة الرئيسية داخل إدارة الصفقة ============

    def on_tick(self, df):
        """
        تُستدعى من SmartPositionManager.manage_position(df)
        ترجع True لو الصفقة أُغلقت بالكامل
        """
        if not self.state["open"] or not self.state.get("profit_engine_active", False):
            return False
        ...
        # حساب R الحالي وتحديث أعلى ربح
        atr_ref = self.atr_entry or max(1e-6, abs(current_price - entry))
        R_now = direction * (current_price - entry) / atr_ref

        highest = self.state.get("highest_profit_pct", 0.0)
        highest = max(highest, R_now)
        self.state["highest_profit_pct"] = highest

        # 1) تريل ديناميكي لو الصفقة قوية وما زالت ماشية
        self._update_trailing(df, float(current_price), R_now)

        # 2) تنفيذ TPs الجزئية
        self._process_tps(float(current_price))

        # 3) التحقق من SL (بعد التريل / BE)
        closed_by_sl = self._check_sl(float(current_price))

        # لو الكمية فضلت صفر بعد TPs ⇒ إغلاق كامل منطقيًا
        if self.state["qty"] <= 0:
            self.state.update(open=False, profit_engine_active=False)
            log_g("✅ ALL TP LEVELS HIT → position fully closed")
            return True
    ...
    def get_status(self):
        return {
            ...
            "tp_levels": [(tp["price"], tp["frac"], tp["label"], tp["hit"]) for tp in self.tp_levels],
