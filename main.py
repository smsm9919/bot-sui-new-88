...
• RF PINE-EXACT - تطابق 100% مع TradingView
"""
...
BOT_VERSION = f"SUI ULTRA PRO AI v7.0 — {EXCHANGE_NAME.upper()} - SMART PROFIT AI + TP PROFILE + COUNCIL STRONG ENTRY + RF PINE-EXACT"
...
# RF MODE - جديد
RF_MODE = os.getenv("RF_MODE", "pine")  # "pine" or "live"
...
# =================== RANGE FILTER - PINE EXACT ===================
def rf_signal_pine_exact(df: pd.DataFrame):
    """
    Pine v4 exact implementation matching TradingView:
      - rng_size: ema(ema(abs(x-x[1]), n), (2n-1))*qty
      - rng_filt: recursive filter
      - fdir persistence
      - longCond/shortCond + CondIni de-dup
    Returns last CLOSED candle signal (like alerts on close).
    """
    if len(df) < RF_PERIOD + 5:
        i = -1
        price = float(df["close"].iloc[i]) if len(df) else 0.0
        t = int(df["time"].iloc[i]) if len(df) else int(time.time() * 1000)
        return {"time": t, "price": price, "long": False, "short": False,
                "filter": price, "hi": price, "lo": price}

    src = df[RF_SOURCE].astype(float)
    
    # 1. rng_size = ema(ema(abs(src - src[1]), RF_PERIOD), (2*RF_PERIOD-1)) * RF_MULT
    diff_abs = (src - src.shift(1)).abs()
    ema1 = diff_abs.ewm(span=RF_PERIOD, adjust=False).mean()
    ema2 = ema1.ewm(span=(2*RF_PERIOD-1), adjust=False).mean()
    rsize = ema2 * RF_MULT
    
    # 2. Range Filter (recursive)
    def _rng_filter_pine(src_series, rsize_series):
        filt = [float(src_series.iloc[0])]
        for i in range(1, len(src_series)):
            prev = filt[-1]
            x = float(src_series.iloc[i])
            r = float(rsize_series.iloc[i])
        ...
        hi = [f + float(rsize_series.iloc[i]) for i, f in enumerate(filt)]
        lo = [f - float(rsize_series.iloc[i]) for i, f in enumerate(filt)]
        
        return pd.Series(hi, index=src_series.index), pd.Series(lo, index=src_series.index), pd.Series(filt, index=src_series.index)
    
    hi, lo, filt = _rng_filter_pine(src, rsize)
    
    # 3. fdir persistence logic (Pine exact)
    fdir = [0]
    for i in range(1, len(filt)):
        f_now = float(filt.iloc[i])
        f_prev = float(filt.iloc[i-1])
    ...
    # 4. upward/downward flags
    upward = [1 if d == 1 else 0 for d in fdir]
    downward = [1 if d == -1 else 0 for d in fdir]
    
    # 5. longCond / shortCond logic
    longCond = [False] * len(df)
    shortCond = [False] * len(df)
    
    for i in range(1, len(df)):
        x = float(src.iloc[i])
        x1 = float(src.iloc[i-1])
        f = float(filt.iloc[i])
        up = upward[i] > 0
        dn = downward[i] > 0
        
        # Pine exact conditions
        longCond[i] = (x > f and x > x1 and up) or (x > f and x < x1 and up)
        shortCond[i] = (x < f and x < x1 and dn) or (x < f and x > x1 and dn)
    
    # 6. CondIni logic (to avoid duplicate signals)
    CondIni = [0] * len(df)
    for i in range(1, len(df)):
        if longCond[i]:
            CondIni[i] = 1
    ...
    # 7. Final conditions (de-dup)
    longCondition = [False] * len(df)
    shortCondition = [False] * len(df)
    
    for i in range(1, len(df)):
        longCondition[i] = longCond[i] and CondIni[i-1] == -1
        shortCondition[i] = shortCond[i] and CondIni[i-1] == 1
    
    # 8. Return last CLOSED candle (like TradingView alerts on close)
    k = -2  # Last closed candle (not current forming one)
    ...
    return {
        ...
        "price": float(src.iloc[k]),
        "long": bool(longCondition[k]),
        "short": bool(shortCondition[k]),
        "filter": float(filt.iloc[k]),
        "hi": float(hi.iloc[k]),
        "lo": float(lo.iloc[k]),
        "upward": upward[k],
        "downward": downward[k],
...
def trade_loop_enhanced_with_smart_patch():
    ...
    while True:
        try:
            ...
            # استخدام RF المناسب
            if RF_MODE == "pine":
                info = rf_signal_pine_exact(df)
                log_i(f"🎯 RF PINE-EXACT: long={info['long']}, short={info['short']}, fdir={info.get('fdir', 0)}")
            else:
                info = rf_signal_live(df)
                log_i(f"🎯 RF LIVE: long={info['long']}, short={info['short']}")
...
def home():
    ...
    return f"✅ SUI ULTRA PRO AI Bot — {EXCHANGE_NAME.upper()} — {SYMBOL} {INTERVAL} — {mode} — Super Council AI + Intelligent Trend Riding + Smart Profit AI + TP Profile System + Council Strong Entry + RF Pine-Exact"
...
def metrics():
    return jsonify({
        ...
        "entry_mode": "SUPER_COUNCIL_AI_GOLDEN_SCALP_SMART_PROFIT_TP_PROFILE_COUNCIL_STRONG_RF_PINE_EXACT", 
...
def verify_execution_environment():
    ...
    print(f"🎯 RF MODE: {RF_MODE.upper()} (Pine-Exact: {RF_MODE=='pine'})", flush=True)
    print(f"🎯 GOLDEN ENTRY: score={GOLDEN_ENTRY_SCORE} | ADX={GOLDEN_ENTRY_ADX}", flush=True)
    print(f"🚀 SMART PATCH: OB/FVG + SMC + Golden Zones + Volume Confirmation + SMART PROFIT AI + TP PROFILE + COUNCIL STRONG ENTRY + RF PINE-EXACT", flush=True)
    print(f"🧠 SMART PROFIT AI: Scalp + Trend + Volume Analysis + TP Profile (1→2→3) + Council Strong Entry + RF Pine-Exact Activated", flush=True)
...
if __name__ == "__main__":
    ...
    log_i(f"🎯 RF MODE: {RF_MODE.upper()} (Pine-Exact: {RF_MODE=='pine'})")
    log_i(f"💡 SMART PATCH ACTIVATED: Golden Zones + SMC + OB/FVG + Zero Reversal Scalping + SMART PROFIT AI + TP PROFILE + COUNCIL STRONG ENTRY + RF PINE-EXACT")
